...
const states = {
  ...
  nextAround: (matrix, stopDownTrigger) => {
    ...
    const addPoints = (store.getState().get('points') + 10) + ((store.getState().get('speedRun') - 1) * 2); // 速度越快, 得分越高
  ...
  clearLines: (matrix, lines) => {
    ...
    const addPoints = store.getState().get('points') + clearPoints[lines.length - 1]; // 一次消除的行越多, 加分越多
    states.dispatchPoints(addPoints);
